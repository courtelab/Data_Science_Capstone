...
movie_avgs <- train_set %>%
  group_by(movieId) %>%
  summarize(b_i = mean(rating - mu), n_i = n()) # Movie effect calculation for each movie
...
model1_rmse <- RMSE(test_set$rating, predicted_ratings)

rmse_results <- bind_rows(rmse_results,
                          data.frame(method = "+ Movie Effect Model", RMSE = model1_rmse, target_RMSE = " < 0.86490"))
...
model2_rmse <- RMSE(test_set$rating, predicted_ratings)
rmse_results <- bind_rows(rmse_results,
                          data.frame(method = "+ User Effect Model", RMSE = model2_rmse, target_RMSE = " < 0.86490"))
...
time_avgs <- train_set %>%
  ...
  summarize(b_t = mean(rating - mu - b_i - b_u), n_t = n())

time_avgs %>% qplot(b_t, geom ="histogram", bins = 20, data = ., color = I("black"))
...
model3_rmse <- RMSE(test_set$rating, predicted_ratings)
rmse_results <- bind_rows(rmse_results,
                          data.frame(method = "+ Time Effect Model", RMSE = model3_rmse, target_RMSE = " < 0.86490"))
...
genre_avgs <- train_set %>%
  ...
  left_join(time_avgs, by = 'ratingPeriod') %>%
  group_by(genres) %>%
  summarize(b_g = mean(rating - mu - b_i - b_u - b_t), n_g = n())

genre_avgs %>% qplot(b_g, geom ="histogram", bins = 20, data = ., color = I("black"))
...
model4_rmse <- RMSE(test_set$rating, predicted_ratings)
rmse_results <- bind_rows(rmse_results,
                          data.frame(method = "+ Genre Effect Model", RMSE = model4_rmse, target_RMSE = " < 0.86490"))
...
model5_rmse <- RMSE(test_set$rating, predicted_ratings)
rmse_results <- bind_rows(rmse_results,
                          data.frame(method = "+ Regularization Effect Model", RMSE = model5_rmse, target_RMSE = " < 0.86490"))
